<x-sidebar-item href="{{ route('dashboard') }}" :isActive=true
    svgPath="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6">
    Dashboard
</x-sidebar-item>
<x-sidebar-item href="#"
    svgPath="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01">
    Forms (Example)
</x-sidebar-item>
<x-sidebar-list-item menuId="inventoryMenu"
    svgPath="m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z"
    title="Inventory" :submenus="[
        ['href' => route('inventory.stock'), 'title' => 'Stock'],
        ['href' => route('inventory.penjualan.index'), 'title' => 'Penjualan'],
        ['href' => route('inventory.pembelian.index'), 'title' => 'Pembelian'],
    ]">
</x-sidebar-list-item>
